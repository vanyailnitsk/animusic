...
const AlbumPage = () => {
    ...
    useEffect(() => {
        getAlbumById(id)
            .then(album => {
                setAlbum(album.data)
                setColors({
                    colorLight: album.data.coverArt.colors.colorLight,
                    colorDark: album.data.coverArt.colors.colorDark
                })
                return album.data
            })
            .catch(error => {
    ...
    return (
        <div className={styles.album__page__wrapper}>
            <div className={styles.album__page__header}
                style={{background: `linear-gradient(to bottom, ${colors.colorLight}, ${colors.colorDark}`}}>
                <div className={styles.album__page__header__content}>
                    ...
                    {album && (
                        <div className={styles.album__main__info}>
                            ...
                            <div>
                                <span className={styles.anime__title}
                                    onClick={() => navigate(animeRoute + album?.anime.id)}>{album?.anime.title}</span>
                                <span> • </span>
            ...
            <div style={{background: `linear-gradient(to bottom, ${colors.colorDark}, #121212`}}
                className={styles.album__page__bottom_rgb}></div>
